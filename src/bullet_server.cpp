...
void *bullet_run(void *arg)
{
    ...
    int nomId = b3sim->loadURDF("/home/robot/robot_ws/RTECAT_Coga/description/hyumm.urdf");
    int actId = b3sim->loadURDF("/home/robot/robot_ws/RTECAT_Coga/description/hyumm.urdf");

    b3sim->setRealTimeSimulation(false);
    b3robot_nom = new Bullet_Hyumm(b3sim,nomId);	
    b3robot_act = new Bullet_Hyumm(b3sim,actId);	
	...
	while(true) 
    {
        ...
        Eigen::Map<MM_JVec> q_sim(bullet_nom->position, MM_DOF_NUM);

        ret = read(sockfd_act, bullet_act, BUFLEN_BULLET);
        Eigen::Map<MM_JVec> q_act(bullet_act->position, MM_DOF_NUM);	
