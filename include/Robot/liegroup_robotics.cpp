...
namespace lr {
	...
	MM_Jacobian JacobianSpace(const MM_ScrewList& Slist, const MM_JVec& thetaList) {
		MM_Jacobian Js = Slist;	
		SE3 T = SE3::Identity();
		for (int i = 1; i < MM_DOF_NUM; i++) {
	...
	MM_Jacobian JacobianBody(const MM_ScrewList& Blist, const MM_JVec& thetaList) {
		MM_Jacobian Jb = Blist;
		SE3 T = SE3::Identity();
		for (int i = MM_DOF_NUM -2; i >= 0; i--) {
	...
	bool IKinBody(const MM_ScrewList& Blist, const SE3& M, const SE3& T,
		...
		while (err && i < maxiterations) {
			...
			Jb = Eigen::Map<Eigen::MatrixXd>(Jb_.data(),6,MM_DOF_NUM);
	...
	bool IKinSpace(const MM_ScrewList& Slist, const SE3& M, const SE3& T,
		...
		while (err && i < maxiterations) {
			...
			Js = Eigen::Map<Eigen::MatrixXd>(Js_.data(),6,MM_DOF_NUM);
	...
	MM_JVec InverseDynamics(const MM_JVec& thetalist, const MM_JVec& dthetalist, const MM_JVec& ddthetalist,
									...
									const vector<Matrix6d>& Glist, const MM_ScrewList& Slist) {
	    // the size of the lists
		int n = MM_DOF_NUM;

		SE3 Mi = SE3::Identity();
		MM_Matrix6xn Ai = MM_Matrix6xn::Zero();
		vector<Matrix6d> AdTi;
		for (int i = 0; i < n+1; i++) {
			AdTi.push_back(Matrix6d::Zero());
		}
		MM_Matrix6xn_1 Vi = MM_Matrix6xn_1::Zero();    // velocity
		MM_Matrix6xn_1 Vdi = MM_Matrix6xn_1::Zero();   // acceleration
	...
	MM_JVec InverseDynamics(const MM_JVec& thetalist, const MM_JVec& dthetalist, const MM_JVec& ddthetalist,
									const Vector3d& g, const Vector6d& Ftip, const vector<SE3>& Mlist,
									const vector<Matrix6d>& Glist, const MM_ScrewList& Slist,double eef_mass) {
	    // the size of the lists
		int n = MM_DOF_NUM;
	...
	MM_JVec GravityForces(const MM_JVec& thetalist, const Vector3d& g,
									const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist) {
	    int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector6d dummyForce = Vector6d::Zero();
		MM_JVec grav = InverseDynamics(thetalist, dummylist, dummylist, g,
                                                dummyForce, Mlist, Glist, Slist);
		return grav;
	}	

	MM_JVec GravityForces(const MM_JVec& thetalist, const Vector3d& g,
									const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist,double eef_mass) {
	    int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector6d dummyForce = Vector6d::Zero();
		MM_JVec grav = InverseDynamics(thetalist, dummylist, dummylist, g,
	...
	MM_MassMat MassMatrix(const MM_JVec& thetalist,
                                const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist) {
		int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero();
		MM_MassMat M = MM_MassMat::Zero();
		for (int i = 0; i < n; i++) {
			MM_JVec ddthetalist = MM_JVec::Zero();
			ddthetalist(i) = 1;
	...
	MM_JVec VelQuadraticForces(const MM_JVec& thetalist, const MM_JVec& dthetalist,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist) {
		int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero(6);
		MM_JVec c = InverseDynamics(thetalist, dthetalist, dummylist,
                             dummyg, dummyforce, Mlist, Glist, Slist);
		return c;
	}	
	
		MM_JVec VelQuadraticForces(const MM_JVec& thetalist, const MM_JVec& dthetalist,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist,double eef_mass) {
		int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		Vector6d dummyforce = Vector6d::Zero(6);
	...
	MM_JVec EndEffectorForces(const MM_JVec& thetalist, const Vector6d& Ftip,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist) {
		int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		MM_JVec JTFtip = InverseDynamics(thetalist, dummylist, dummylist,
                             dummyg, Ftip, Mlist, Glist, Slist);
		return JTFtip;
	}
	
		MM_JVec EndEffectorForces(const MM_JVec& thetalist, const Vector6d& Ftip,const vector<SE3>& Mlist, const vector<Matrix6d>& Glist, const MM_ScrewList& Slist,double eef_mass) {
		int n = MM_DOF_NUM;
		MM_JVec dummylist = MM_JVec::Zero();
		Vector3d dummyg = Vector3d::Zero();
		MM_JVec JTFtip = InverseDynamics(thetalist, dummylist, dummylist,
	...
	MM_JVec ForwardDynamics(const MM_JVec& thetalist, const MM_JVec& dthetalist, const MM_JVec& taulist,
		...
		MM_JVec totalForce = taulist - VelQuadraticForces(thetalist, dthetalist, Mlist, Glist, Slist)
                 							 - GravityForces(thetalist, g, Mlist, Glist, Slist)
                                             - EndEffectorForces(thetalist, Ftip, Mlist, Glist, Slist);
		MM_MassMat M = MassMatrix(thetalist, Mlist, Glist, Slist);
		// Use LDLT since M is positive definite
	    Eigen::LDLT<MM_MassMat> ldlt(M);
		Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd>(totalForce.data(), totalForce.size());
		Eigen::VectorXd x = ldlt.solve(v);
        // MM_JVec ddthetalist = M.ldlt().solve(totalForce);
		MM_JVec ddthetalist =  Eigen::Map<MM_JVec>(x.data(), x.size());
		return ddthetalist;
	}	
	MM_JVec ForwardDynamics(const MM_JVec& thetalist, const MM_JVec& dthetalist, const MM_JVec& taulist,
									const Vector3d& g, const Vector6d& Ftip, const vector<SE3>& Mlist,
		...
		MM_JVec totalForce = taulist - VelQuadraticForces(thetalist, dthetalist, Mlist, Glist, Slist,eef_mass)
                 							 - GravityForces(thetalist, g, Mlist, Glist, Slist,eef_mass)
                                             - EndEffectorForces(thetalist, Ftip, Mlist, Glist, Slist,eef_mass);
		MM_MassMat M = MassMatrix(thetalist, Mlist, Glist, Slist,eef_mass);
		// Use LDLT since M is positive definite
	    Eigen::LDLT<MM_MassMat> ldlt(M);
		Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd>(totalForce.data(), totalForce.size());
		Eigen::VectorXd x = ldlt.solve(v);
	...
	void FKinBody(const SE3& M,const MM_ScrewList& Blist, const MM_JVec& q ,const MM_JVec& dq, SE3 &T, MM_Jacobian &Jb,MM_Jacobian& dJb){
		Jb = Blist;
		dJb = MM_Jacobian::Zero();
		T = SE3::Identity();
		SE3 T_ = SE3::Identity();
		Vector6d prev_dJidt=Blist.col(MM_DOF_NUM-1)*dq(MM_DOF_NUM-1);
		for(int i = MM_DOF_NUM-2 ;i >= 0;i--){	
       ...
       MM_Jacobian dJacobianBody(const SE3& M,const MM_ScrewList& Blist, const MM_JVec& q ,const MM_JVec& dq){
		MM_Jacobian Jb = Blist;
		MM_Jacobian dJb = MM_Jacobian::Zero();
		SE3  T = SE3::Identity();
		SE3 T_ = SE3::Identity();
		Vector6d prev_dJidt=Blist.col(MM_DOF_NUM-1)*dq(MM_DOF_NUM-1);
		for(int i = MM_DOF_NUM-2 ;i >= 0;i--){	
...
void JointTrajectory(const MM_JVec q0, const MM_JVec qT, double Tf, double t , int method , MM_JVec& q_des, MM_JVec& q_dot_des, MM_JVec& q_ddot_des) {
	...
	// RelMM_Jacobian RelJacobianSpace(const RelMM_ScrewList& Slist, const RelMM_JVec& thetaList) {
	// 	RelMM_Jacobian Js = Slist;	
	// 	SE3 T = SE3::Identity();
	...
	// 	return Js;
	// }
	// RelMM_Jacobian RelJacobianBody(const RelMM_ScrewList& Blist, const RelMM_JVec& thetaList) {
	// 	RelMM_Jacobian Jb = Blist;
	// 	SE3 T = SE3::Identity();
    ...
    //    RelMM_Jacobian ReldJacobianBody(const SE3& M,const RelMM_ScrewList& Blist, const RelMM_JVec& q ,const RelMM_JVec& dq){
	// 	RelMM_Jacobian Jb = Blist;
	// 	RelMM_Jacobian dJb = RelMM_Jacobian::Zero();
	// 	SE3  T = SE3::Identity();
	// 	SE3 T_ = SE3::Identity();
	...
	// }		
	// bool RelIKinSpace(const RelMM_ScrewList& Slist, const SE3& M, const SE3& T,
	// 	RelMM_JVec& thetalist, double eomg, double ev) {
	// 	int i = 0;
	// 	int maxiterations = 20;
	...
	// bool RelIKinBody(const RelMM_ScrewList& Blist, const SE3& M, const SE3& T,
	// 	RelMM_JVec& thetalist, double eomg, double ev) {
	// 	int i = 0;
	// 	int maxiterations = 20;
